...
app.get('/api/admin/dashboard', auth('admin'), (req, res) => {
  const data = loadData();
  const employees = data.employees.map(e => ({ id:e.id,name:e.name,email:e.email,phone:e.phone||'',active:e.active,created_at:e.created_at }));
  const today = day();
  const attendance = data.attendance.filter(a => a.date === today).map(a => { const e=data.employees.find(x=>x.id===a.employee_id)||{}; return {...a,name:e.name||'Unknown',email:e.email||'',phone:e.phone||'',...calc(a)}; }).sort((a,b)=>a.name.localeCompare(b.name));
  ...
  const onLeave = activeEmployees.filter(e => todayLeaveIds.has(e.id) && !workStartedIds.has(e.id)).length;
  const notStartedEmployees = activeEmployees.filter(e => !workStartedIds.has(e.id) && !todayLeaveIds.has(e.id)).map(e => ({ id:e.id,name:e.name,email:e.email,phone:e.phone||'' }));
  const remaining = notStartedEmployees.length;
  const leaves = data.leaves.filter(l => l.status === 'pending').map(l => { const e=data.employees.find(x=>x.id===l.employee_id)||{}; return {...l,employee_name:e.name||'Unknown',employee_email:e.email||'',employee_phone:e.phone||''}; }).sort((a,b)=>b.created_at.localeCompare(a.created_at));
  res.json({ employees, attendance, pending_leaves: leaves, not_started_employees: notStartedEmployees, date: today, stats: { total_employees: activeEmployees.length, work_started: workStarted, remaining, on_leave: onLeave } });
});
app.post('/api/admin/employees', auth('admin'), (req,res)=>{ const data=loadData(); const name=String(req.body.name||'').trim(); const email=String(req.body.email||'').trim().toLowerCase(); const phone=String(req.body.phone||'').trim(); const password=String(req.body.password||''); if(!name||!email||!password)return res.status(400).json({error:'Name, email and password are required'}); if(data.employees.some(e=>e.email===email))return res.status(400).json({error:'Email already exists'}); const e={id:nextId(data.employees),name,email,phone,password,active:true,created_at:iso()};data.employees.push(e);saveData(data);res.json({id:e.id}); });
app.patch('/api/admin/employees/:id', auth('admin'), (req,res)=>{ const data=loadData(); const e=data.employees.find(x=>x.id===Number(req.params.id)); if(!e)return res.sendStatus(404); const email=String(req.body.email??e.email).trim().toLowerCase(); if(data.employees.some(x=>x.id!==e.id&&x.email===email))return res.status(400).json({error:'Email already exists'}); e.name=String(req.body.name??e.name).trim();e.email=email;e.phone=String(req.body.phone??e.phone??'').trim();if(req.body.password)e.password=String(req.body.password);if(req.body.active!==undefined)e.active=!!req.body.active;saveData(data);res.json({ok:true}); });
app.get('/api/admin/attendance', auth('admin'), (req,res)=>{ const data=loadData(); let rows=data.attendance.slice(); if(req.query.month&&/^\d{4}-\d{2}$/.test(req.query.month))rows=rows.filter(a=>a.date.startsWith(req.query.month));if(req.query.employee_id)rows=rows.filter(a=>a.employee_id===Number(req.query.employee_id));rows=rows.map(a=>{const e=data.employees.find(x=>x.id===a.employee_id)||{};return{...a,name:e.name||'Unknown',email:e.email||'',phone:e.phone||'',...calc(a)};}).sort((a,b)=>b.date.localeCompare(a.date)||a.name.localeCompare(b.name));res.json(rows); });
